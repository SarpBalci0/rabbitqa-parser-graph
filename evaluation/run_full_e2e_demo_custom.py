...
Copy of run_full_e2e_demo.py modified to use a caller-supplied source file
(/tmp/my_test/article.txt) instead of the built-in fixture text, per user
request. See run_full_e2e_demo.py's own docstring for the full §9.4 rationale;
this file only differs in STAGE 1 (source selection) and the derived
clause_id/control naming that follows from the actual article/paragraph number
in that file, rather than hardcoding a second copy of the demo text.
"""
...
SOURCE_FILE = Path("/tmp/my_test/article.txt")
...
def _derive_structural_path(source_text: str) -> str:
    """Reads the article/paragraph numbers straight out of the source file
    instead of hardcoding "article-21/paragraph-1", so this script works for
    whatever article/paragraph number SOURCE_FILE actually contains."""
...
def main() -> int:
    ...
    clause_id = f"{document_payload['document_id']}:v1-custom-2026-09-01:{structural_path}"
    ...
    print(f"All eight stages ran as one reproducible script against {SOURCE_FILE}, no manual data patching:")
