...
| Interactive proof-path visualization (pan/zoom/click-through, editable diagrams, a JS diagramming framework) | v1's proof-path viewer (§4.5, §5.11) renders static SVG only, generated server-side from an already-published, already-queried result — no client-side interactivity or diagram-editing capability exists or is implied (added spec_version 1.2.0, §12 changelog) |
...
- **Mid-word hard-break detection (spec_version 1.1.2, refined 1.1.3).** The confidence-scoring step MUST detect a lowercase letter immediately followed by a line break immediately followed by a lowercase letter, with no intervening space or hyphen (e.g. PDF layout wrapping "remote" as "remo" + line break + "te") — this pattern is a signal of a broken word, distinct from the already-required hyphenation-break case (`<letter>-\n<letter>`), since it carries no hyphen to key off. Detecting a genuine instance of this pattern MUST lower `extraction_metadata.confidence` and add an entry to `extraction_metadata.warnings`, so it surfaces at the §7 ingress quality gate exactly as any other extraction-quality signal does. The extraction step MUST NOT attempt to automatically rejoin the split word (or any other detected artifact) — doing so would risk silently altering legal text content (e.g. wrongly rejoining a legitimately line-final word with the next line's first word), which conflicts with this section's `char_start`/`char_end`-and-content-fidelity invariant above. Flag, never silently correct.
  - **Raw pattern presence is an unacceptably weak signal on its own (spec_version 1.1.3).** An empirical scan against 491 real, successfully-extracted PDFs (a personal-machine corpus, mostly lecture-note/slide decks, plus one real prose-style report PDF) found that of 19,938 raw pattern matches, only 32 were a genuinely broken word — the other ~99.8% were two ordinary, correctly-spelled adjacent words that the extraction library had not separated with a space at the line-wrap boundary (e.g. `"a"`+`"condition"`, `"computer"`+`"main"`). Under presence-only detection this pushed the warning/confidence-penalty onto 417 of 491 files (85%) — nearly every multi-page PDF, defeating the signal's purpose. The detection step MUST therefore disambiguate a genuine break from two adjacent real words before penalizing confidence — e.g. by checking whether the rejoined fragment is a real word while the left fragment alone is not (a dictionary-based check; see `clause_parser/src/canonicalize/pdf_extractor.py`'s `_classify_broken_word_matches`) — rather than penalizing on raw pattern presence alone. Re-running the same 491-file scan with this disambiguation reduced flagged files from 417 to 30 (and raw matches counted from 19,938 to 33), while still catching the same known-genuine cases. Where no such disambiguation resource is available at runtime, the step MUST still run the raw presence-only check as a conservative fallback (never silently skip detection) but SHOULD note in the warning text that precision is degraded, since a per-host resource dependency is itself a documented determinism trade-off: byte-identical PDF input MAY score differently across hosts depending on local resource availability, which callers relying on this score for cross-environment reproducibility MUST account for.
...
### 4.5 Proof-Path Visualization renderer (added spec_version 1.2.0)

A read-only rendering module that turns one obligation's already-computed §3.3 proof-path result into a static SVG diagram embedded in a minimal HTML page, for human inspection (e.g. during review or audit) — no new graph traversal, no new data source.

| Aspect | Rule |
|---|---|
| Data source | MUST call the existing `run_proof_path_query` (`compliance_graph/src/query/proof_path.py`, §5.9's query logic) against the requested `snapshot_id`, then select the single result whose `clause_id` matches the request. MUST NOT implement or duplicate any graph traversal of its own. |
| Node rendering | Each node in the result's `path` array is rendered as a labeled box. `path` entries are `"{node_type}:{id}"` strings (e.g. `"control:ctrl-mfa-remote-access"`); the box label MUST show the type (from the prefix) and a short identifier (the suffix, truncated for display only — never truncated in a way that changes the underlying `clause_id`/`node_id` used elsewhere). |
| Edge rendering | `path` is an ordered node list, not an edge list — it carries no explicit relationship-type data. Edge labels between consecutive nodes MUST be derived deterministically from §3.3's fixed canonical path shape (Provision --DERIVED_FROM--> Obligation --MAPS_TO_CONTROL--> Control --AFFECTS_ASSET--> Asset; Control --SATISFIED_BY--> EvidenceRequirement --EVIDENCED_BY--> TestAsset), using §3.2's relationship names verbatim. This renderer MUST NOT be used for any path shape other than §3.3's canonical proof-path — a future, differently-shaped path pattern requires its own edge-labeling rule, not a guess. |
| Obligation text | The result's `verbatim_text` MUST be rendered alongside the diagram, unmodified in meaning (whitespace-safe display formatting only — same content-fidelity bar as §2.1's canonicalization invariant). |
| Determinism | Fully deterministic, no LLM call — the same query result MUST always render to byte-identical SVG/HTML output. |
| Output sanitization | See §7's new "Visualization rendering" trust-zone row — this is a hard requirement, not a rendering-quality nicety. |
| Interactivity | Static output only for v1: no `<script>`, no event-handler attributes, no `<foreignObject>`, no external resource references. No click/drag/zoom. This is a v1 scope boundary, not a technical ceiling — see §1.2 non-goals. |
...
### 5.11 `GET /v1/graph/snapshots/{snapshot_id}/proof-path-view` (added spec_version 1.2.0)
Request: query parameter `clause_id: str` (required).
Response `200`: `Content-Type: text/html`. Body is a minimal, self-contained HTML page (no external stylesheet/script references) containing one inline `<svg>` rendering the §3.3 proof-path for the requested obligation (§4.5), plus its `verbatim_text` shown alongside the diagram.

All error responses from this endpoint (`400`, `404`) follow §5's preamble error envelope (`{ "error": { "code", "message", "details" } }`) as `application/json`, identically to every other endpoint in this spec — this endpoint's `200` is the only response from it with an HTML body; a `400`/`404` from it is never an HTML error page.

Rules:
- MUST reuse §5.9's `proof_path` query logic (`run_proof_path_query`) against `snapshot_id`, then select the entry whose `clause_id` matches the request — no independent graph traversal (§4.5).
- Same review-gate rule as §5.9/§2.5: this endpoint sources data ONLY from an already-published, already-approved snapshot's query results. It MUST NOT render data from a `draft`/`validated`/`approved`-but-unpublished changeset, and MUST NOT render an obligation whose `review_status` is not `accepted`.
- `404` (JSON envelope) if `snapshot_id` is not fully published (reuses §5.9's "querying anything not fully published returns 404, not partial data" rule verbatim).
- `404` (JSON envelope) if `snapshot_id` is published but no proof-path result exists for the given `clause_id` in that snapshot (obligation not present, not mapped to a control/asset/evidence chain yet, or filtered out by the review-gate rule above) — same "404, not partial data" principle: never render a partial diagram for an obligation that doesn't have a complete, source-backed proof-path.
- `400` (JSON envelope) if `clause_id` is missing from the request.

All text values interpolated into the SVG/HTML (node labels, the `clause_id`, `verbatim_text`) MUST be escaped per §7's new "Visualization rendering" trust-zone row before being embedded — `verbatim_text` in particular originates from an untrusted source document (§7 Document ingress) and MUST be treated as untrusted content at every rendering boundary, not only at ingress.
...
| Visualization rendering (added spec_version 1.2.0) | Untrusted-content sink | The §4.5/§5.11 proof-path viewer embeds node labels and `verbatim_text` — content that ultimately originates from an untrusted source document (§7 Document ingress) — into SVG/HTML output. Every such value MUST be escaped for the XML/HTML context (at minimum `&`, `<`, `>`, `"`, `'`) immediately before insertion, using a library or function whose only job is context-correct escaping — never manual string replacement, and never an f-string/concatenation that interleaves untrusted text with markup without going through that escaping step. The renderer MUST NOT emit `<script>`, `<foreignObject>`, `on*` event-handler attributes, `javascript:`/`data:` URIs, or any other executable or externally-resolvable content — output is inert markup only (boxes, text, lines) by construction, not by a denylist filter over otherwise-unrestricted SVG. This is a hard gate: if a value cannot be safely escaped for the output context, the request MUST fail (500, logged) rather than emit unescaped content. |
...
| §4.5/§5.11 Proof-Path Visualization | N/A — feature not in source report, spec-only addition (spec_version 1.2.0) |
...
| 1.2.0 | 2026-09-03 | Feature addition: visual proof-path viewer. New §4.5 defines a deterministic, no-LLM rendering module that turns an already-computed §3.3 proof-path query result (reusing `run_proof_path_query`, §5.9 — no new graph traversal) into a static SVG diagram embedded in a minimal HTML page: labeled node boxes (type + short id, from each `path` entry's `"{node_type}:{id}"` shape) connected by edges labeled with §3.2's relationship names, derived deterministically from §3.3's fixed canonical path shape (the `path` array carries node order only, no explicit edge/relationship data) since this renderer is scoped to that one canonical shape only. New §5.11 `GET /v1/graph/snapshots/{snapshot_id}/proof-path-view?clause_id=...` endpoint, reusing §5.9's "not fully published → 404, never partial data" rule and §2.5's accepted-only review gate. New §7 trust-zone row, "Visualization rendering": node labels and `verbatim_text` ultimately originate from an untrusted source document (§7 Document ingress) and MUST be escaped for the SVG/XML context at the point of embedding — via a dedicated escaping function, never manual string interpolation — with no `<script>`/`<foreignObject>`/event-handler/executable-URI content permitted by construction. New §1.2 non-goal: interactive (pan/zoom/click-through/editable) visualization is out of v1 scope — static SVG only. Module placement: a new `compliance_graph/src/visualization/` submodule for the deterministic renderer, with its route in `compliance_graph/src/api/`, following the existing `export/` module's split between rendering logic and its thin API wrapper. §5.11 explicitly states its `400`/`404` error responses follow §5's preamble JSON error envelope (`{ "error": { "code", "message", "details" } }`) identically to every other endpoint — only its `200` success response has an HTML body — since this endpoint is otherwise the spec's first to mix an HTML success response with JSON-shaped errors and that mixture is not obvious without stating it. Minor version bump per §8 (new capability, additive — no existing schema, endpoint, or acceptance criterion changed; `ontology_version` unaffected since §3.1/§3.2 tables are unchanged, only read from). §11 Traceability Matrix gains an explicit "N/A, spec-only addition" row for §4.5/§5.11 rather than leaving this feature silently absent from that matrix, since this feature has no corresponding source-report section — it originates directly from this spec, not from the original Implementation Report. |
| 1.1.3 | 2026-09-03 | Gap fix, empirically driven: 1.1.2's mid-word hard-break detection (`_BROKEN_WORD_RE`, presence-only) was tracked as a known gap with an unbounded false-positive risk (see README "Other known, explicitly-tracked gaps"). Empirically bounding it against 491 real, successfully-extracted PDFs (personal-machine corpus, mostly lecture slides, plus one real prose-style report PDF) found the false-positive rate was severe, not marginal: of 19,938 raw pattern matches, only 32 were genuinely broken words (99.8% false positives among classifiable matches), and presence-only detection flagged 417 of 491 files (85%) — the check fired on nearly every real PDF, not just ones with a genuine broken word. §2.1's mid-word hard-break invariant now explicitly requires disambiguating a genuine break from two ordinary adjacent words (ordinary PDF line-wrap frequently omits the inter-word space pypdf would need to tell them apart) before penalizing confidence, with a documented conservative fallback (and its own determinism caveat) when no disambiguation resource is available. `clause_parser/src/canonicalize/pdf_extractor.py`'s `_classify_broken_word_matches` implements this via a dictionary check (rejoined fragment is a real word, left fragment alone is not); re-running the same 491-file scan against the fixed implementation dropped flagged files from 417 to 30 and counted matches from 19,938 to 33, while still catching known-genuine cases (e.g. `remo`+`te`→`remote`). No required field, endpoint, or acceptance criterion changed; `CanonicalDocument.schema_version` const is unchanged at `"1.1.0"` (same precedent as 1.1.1/1.1.2). |
